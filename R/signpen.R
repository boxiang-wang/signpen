signpen = function(x, y, nlambda=100, method = c("ls", "logit"), 
  lambda.factor=ifelse(nobs < nvars, 0.01, 1e-04), 
  lambda=NULL, lambda2=0, lamdba3=1, pf=rep(1, nvars), 
  ...
  ####################################################################
  fit = switch(method, 
    ls = lspath(x, y, nlam, flmin, ulam, isd, eps, 
      dfmax, pmax, jd, pf, pf2, maxit, strong, 
      lam2, lam3, gam, nobs, nvars, vnames), 
    logit = logpath(x, y, nlam, flmin, ulam, isd, eps, 
      dfmax, pmax, jd, pf, pf2, maxit, strong, 
      lam2, lam3, gam, nobs, nvars, vnames), 
    )
  if (is.null(lambda)) 
...
lspath = function(x, y, nlam, flmin, ulam, isd, eps, dfmax, pmax, 
  jd, pf, pf2, maxit, strong, lam2, lam3, gam, nobs, nvars, vnames) {
  ####################################################################
  #data setup
  y = as.numeric(y)
  ####################################################################
...
logpath = function(x, y, nlam, flmin, ulam, isd, eps, dfmax, pmax, 
  jd, pf, pf2, maxit, strong, lam2, lam3, gam, nobs, nvars, vnames) {
  ####################################################################
  #data setup
  y = as.factor(y)
